...
//
///Input is 8x8 byte matrix. It's data is shifted in to a byte per row. \n
...
/// this makes it reuseble you can ignore the first adress and only use the data, then pare it ypu self to other adresses. \n
        mbin_to_mhex(byte (*matrix)[8], byte (*matrix_data)[2], int inverd=0):
        ...
        ///The first number put in to matrix_data is the row number, second value is the column data. \n
        {
            ...
            {
                byte matrix_tmp[8][8];                                  // Temp matix to store and alter data.
                byte tmp = 0;                                           // An temp byte, in to this byte data will be shifted to make from an matrix row, 1 byte.
